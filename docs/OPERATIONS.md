...
With gallery photos set, the hero becomes a full-bleed crossfade slideshow (Ken Burns zoom, dot navigation) instead of the plain gradient; the gallery grid itself opens a click-through lightbox. A "Popular dishes" section pulls `is_popular` menu items automatically — no separate setting, just tag dishes popular from the menu editor. Every one of these sections (and the service-highlights bar: dine-in/pickup/delivery + today's hours) only renders when there's content for it, so a freshly created restaurant's home page looks exactly like it always has until its owner fills something in. Sections fade in on scroll (`v-reveal`, `app/plugins/reveal.ts`) — a one-time IntersectionObserver-driven animation that respects `prefers-reduced-motion` and never causes a hydration mismatch (the animation class is only ever added client-side, after hydration).
