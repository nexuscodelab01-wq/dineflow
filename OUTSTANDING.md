...
### 0. Home page content follow-ups
- No geocoding: coordinates for the map are entered by hand (via a "find yours on Google Maps" link), not looked up from the typed address. Fine for a handful of restaurants; add real geocoding (and keep it in sync when the address changes) if this needs to scale.
- No image reordering or captions in the gallery — photos show in upload order.
- The home page hero still uses the short `description` field; `about_text` is a separate, longer field shown further down. Some restaurants will duplicate themselves between the two — worth a hint in Settings if it comes up.

### 1. Embeddable widget and table combining follow-ups
...
### 6. Sessions after a password change
...
### 7. Row-level security follow-ups
...
### 10. Reservation-link follow-ups
...
### 12. Platform console follow-ups
